(function(){
  ...
  function mixin(obj) { // 将Emitter中的prototype表的每一项复制到obj表里面
  ...
  Emitter.prototype.addEventListener = function(event, fn){
    this._callbacks = this._callbacks || {}; //  true:this._callbacks  false:{}
  ...
  Emitter.prototype.once = function(event, fn){
    ...
    function on() {
      self.off(event, on);
      fn.apply(this, arguments); // arguments参数
    }
  ...
  Emitter.prototype.removeEventListener = function(event, fn){
    ...
    // specific event // 移除特殊的监听事件
    var callbacks = this._callbacks[event];
    if (!callbacks) return this;

    // remove all handlers // 取消所有的监听
    if (1 == arguments.length) {
      delete this._callbacks[event];
      return this;
    }

    // remove specific handler // 移除指定的监听事件
    var cb;
  ...
  Emitter.prototype.emit = function (event) {
    ...
    if (callbacks) {
      ...
      for (var i = 0, len = callbacks.length; i < len; ++i) {
        callbacks[i].apply(this, args); // 和call类似
      }
    }
...
(function (exports, ByteArray, global) {
  ...
  Protocol.strencode = function(str) {
    ...
    for(var i = 0; i < str.length; i++){
      var charCode = str.charCodeAt(i); // 返回指定位置的字符的 Unicode 编码
      var codes = null;
      if(charCode <= 0x7f){
        codes = [charCode];
  ...
  Protocol.strdecode = function(buffer) {
    ...
    return String.fromCharCode.apply(null, array); // fromCharCode可接受一个指定的 Unicode 值，然后返回一个字符串
  };
  ...
  Message.decode = function(buffer) {
    var bytes =  new ByteArray(buffer); // 将收到的数组转化为二进制
...
(function (exports, global){
  ...
  Codec.decodeStr = function(bytes, offset, length){
    ...
    for(var i = 0; i < array.length;){
      str += String.fromCharCode.apply(null, array.slice(i, i + 10000)); // String.fromCharCode 把ascii码转成字符串
      i += 10000;
    }
    ...
    for(var name in msg){
      if(!!protos[name]){
        ...
        switch(proto.option){
          case 'required' :
          case 'optional' :
            offset = writeBytes(buffer, offset, encodeTag(proto.type, proto.tag)); // 将（）里的string写成字节
    ...
    switch(type){
      ...
      default :
        ...
        if(!!message){
          ...
          var tmpBuffer = new ArrayBuffer(codec.byteLength(JSON.stringify(value))*2); // 字节长度
          var length = 0;
...
(function() {
  ...
  var connect = function(params, url, cb) {
    ...
    if(window.localStorage && window.localStorage.getItem('protos') && protoVersion === 0) {
      ...
      if(!!decodeIO_protobuf) {
        decodeIO_encoder = decodeIO_protobuf.loadJson(clientProtos); //JQuery loadJSON plugin是一个插件能够让开发人员从服务器加载JSON数据并将JSON对象装载到DOM中
  ...
  var processPackage = function(msgs) {
    if(Array.isArray(msgs)) { // 判断是否是isArray
